...
#-------------------------------------------------------------------------------------
#internal function for 'mbr.association'
##it creates a matrix of regulons (tnet) for analysis (can be numeric or factors)
...
#-------------------------------------------------------------------------------------
#internal function for 'mbr.association'
##it gets significant associations inferred for motifs using the 'prob' parameter
.motifsquantile <- function(regcor, th=0.99)
{
  ...
  qvalues <- data.frame(Regulon1=rnames,Regulon2=cnames,R=corvalues, Quantile = qvalues, stringsAsFactors=FALSE)
...
.cutoffquantile <- function (regcor)
{
  ...
  cormat <- matrix(cormat, ncol=ncol(regcor), nrow=nrow(regcor), dimnames=list(rownames(regcor), colnames(regcor)))
  return(cormat)
}
...
.jc.overlap <- function(regEle1, regEle2, tnet, overlap="all"){
  ...
  amap <- apply(tnet[, regEle1, drop=FALSE], 2, jc, xmat=tnet[, regEle2, drop=FALSE])
  return(amap)
}
...
#-------------------------------------------------------------------------------------
#internal function for 'mbr.association'
##it gets mutual information between 'duals' regulatory elements and the p-value when available
.getMIorP <- function(pvlist, regel, tbmi, size1, size2, mutualInformation=TRUE, cutoff=0)
{
  tb <- as.matrix(pvlist[, c("Regulon1", "Regulon2")])
  ...
  if(mutualInformation)
      {
          pvlist <- cbind(pvlist, MI=mutinf[1, ], Size.Regulon1=mutinf[2, ],Size.Regulon2=mutinf[3, ])
          pvlist <- pvlist[, c("Regulon1", "Size.Regulon1", "Regulon2", "Size.Regulon2", "MI", "R", "Quantile")]
          ##----MI
  ...
  else
      {
          ...
          pvlist$MI.Adjusted.Pvalue[pvlist$MI.Adjusted.Pvalue<cutoff] <- paste("<", cutoff, sep="")
          pvlist <- pvlist[, c("Regulon1", "Size.Regulon1", "Regulon2", "Size.Regulon2", "MI", "MI.Adjusted.Pvalue", "R", "Quantile")]
      }
  return (pvlist)
}
...
.mbr.hyper <- function(pvlist, regulons, regel, universe, pAdjustMethod, verbose=TRUE)
{
  ...
  for(i in 1:nrow(regpairstb))
  {
    ...
    tmp <- .regulon.hyper(regulon1=regulon1, universe=universe, regulon2=regulon2)
    res <- rbind(res, tmp)
  }
  if(verbose) close(pb)
...
.combineTNIs <- function (tni1, tni2, verbose = TRUE)
{
  ...
  if(any(tni1@status[1:4] != "[x]") || any(tni2@status[1:4] != "[x]"))
  {
    ...
    stop("Both TNIs must be evaluated by the RTN pipeline up to the DPI filter.")
  }
}
...
.consisSuppTable <- function(object, supplementary.table, evidenceColname, verbose)
{
  ##-----checks
  if(!evidenceColname%in%colnames(supplementary.table)) stop("'evidenceColname' should be a valid colname in 'supplementary.table", call.=FALSE)
  ##---
  idx <- which(colnames(supplementary.table)%in%evidenceColname)
  if(idx!=3) stop("'evidenceColname' should be the third column in 'supplementary.table", call.=FALSE)
  ##---
  ...
  idx <- which(colnames(supplementary.table)%in%evidenceColname)
  tni1 <- mbr.get(object, what="TNI1"); annot <- tni1@annotation; annot <- as.matrix(annot)
  ...
  if(consc<90) warning(paste("Only",paste(round(consc,2),"%",sep=""), "of 'supplementary.table' is listed in the annotation!\n"), call.=FALSE)
  if(consc>90 & verbose) cat(paste("-",paste(round(consc,2),"%",sep=""), "of 'supplementary.table' is listed in the annotation!\n"))
  ##-----
  return(tmp)
}
#-------------------------------------------------------------------------------------
...
.checkLoops <- function (object, supplementary.table, evidenceColname, verbose=TRUE)
{
    motifsInformation <- mbr.get(object, what="motifsInformation")
    motifsInformation[, evidenceColname] <- NA
    ##---
    if(verbose) cat("-Checking whether evidences in 'supplementary.table' support the existence of the inferred duals...\n")
    if(verbose)pb<-txtProgressBar(style=3)
    x <- 0
    ##---
    ...
    for (i in 1:length(regs1))
      {
          rg1 <- regs1[i]
          idx1 <- which((supplementary.table[,1] %in% rg1) | (supplementary.table[,1] %in% names(rg1)))
          idx2 <- which((supplementary.table[,2] %in% rg1) | (supplementary.table[,2] %in% names(rg1)))
          ...
          nmsDuals <- paste(tpev[,1], tpev[,2],sep="~"); rownames(tpev) <- nmsDuals
